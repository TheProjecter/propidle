package com.googlecode.propidle.security;

import com.googlecode.propidle.authentication.AuthenticatedRequestRouter;
...
import com.googlecode.propidle.authorisation.AuthorisingHttpHandler;
import com.googlecode.totallylazy.*;
import com.googlecode.utterlyidle.*;
import com.googlecode.utterlyidle.handlers.ResponseHandlersFinder;

import static com.googlecode.propidle.authentication.AuthenticationResource.ORIGINATING_REQUEST_FORM_NAME;
import static com.googlecode.propidle.authentication.AuthenticationResource.UTTERLYIDLE_AUTHENTICATE;
...
public class SecurityHttpHandler implements HttpHandler {
    ...
    private Response loginAndHandle(final Request request, final Option<String> originatingRequestAsString) throws Exception {
        if (username(request).value().isEmpty()) {
            return denied(request, originatingRequestAsString).call(Denial.denial(username(request).value()));
        }

        final Either<Denial, com.googlecode.propidle.authentication.api.Identity> authenticationResult = authenticator.authenticate(username(request), password(request), request);
    ...
    private Callable1<com.googlecode.propidle.authentication.api.Identity, Response> authenticated(final Request request, final Option<String> originatingRequestAsString) {
        return new Callable1<com.googlecode.propidle.authentication.api.Identity, Response>() {
            public Response call(com.googlecode.propidle.authentication.api.Identity identity) throws Exception {
