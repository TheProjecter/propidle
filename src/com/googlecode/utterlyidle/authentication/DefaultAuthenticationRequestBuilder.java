...
import com.googlecode.utterlyidle.authentication.api.AuthenticationRequestBuilder;
...
import static com.googlecode.utterlyidle.authentication.SessionCookieGrantingHttpHandler.SESSION_COOKIE_NAME;
...
public class DefaultAuthenticationRequestBuilder implements AuthenticationRequestBuilder {
    ...
    public Request buildAuthenticationRequest(Option<Request> originatingRequest, Option<Integer> failedLoginCount) {
        ...
        try {
            return get(redirector.uriOf(method(on(AuthenticationResource.class).authenticationPage(null, encodedRequest, failedLoginCount)))).cookie(cookie(SESSION_COOKIE_NAME, sessionCookie)).build();
        } catch (IOException e) {
    ...
    private Callable1<Request, String> sessionCookie() {
        return new Callable1<Request, String>() {
            public String call(Request request) throws Exception {
                return cookies(request.headers()).getValue(SESSION_COOKIE_NAME);
            }
        };
    }
